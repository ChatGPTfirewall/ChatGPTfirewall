...
class HierarchicalSummarizer:
    def __init__(self, model_name="facebook/bart-large-cnn", device=None):
        ...
        if "allenai/longformer-base-4096" in model_name:
            self.tokenizer = AutoTokenizer.from_pretrained(model_name)
            self.model = EncoderDecoderModel.from_pretrained(model_name).to(self.device)

        elif "bigbird-pegasus" in model_name:
            self.tokenizer = AutoTokenizer.from_pretrained(model_name)
            self.model = BigBirdPegasusForConditionalGeneration.from_pretrained(model_name).to(self.device)

        else:
            # Default: BART, DistilBART, PEGASUS
            self.tokenizer = AutoTokenizer.from_pretrained(model_name)
